...
SD_Error SD_Init(void)
{
	...
	printf("SD card power on ing..\n\r");
	/*上电并进行卡识别流程，确认卡的操作电压  */
	errorstatus = SD_PowerON();
	if(errorstatus == SD_OK)	//test
	{
		printf("SD card power on success\n\r");
	}

	/*如果上电，识别不成功，返回“响应超时”错误 */
	if (errorstatus != SD_OK)
	{
		printf("SD card power on errorstatus: %d \n\r", errorstatus);	//test
...
#if _FATFS
#define SD_READ_BUFF_SIZE				1024
#define MP3_SD_BUFF						SD_READ_BUFF_SIZE
FATFS fs;													/* FatFs文件系统对象 */
FIL fnew;													/* 文件对象 */
FRESULT res_sd;                /* 文件操作结果 */
UINT fnum;            					  /* 文件成功读写数量 */
BYTE ReadBuffer[SD_READ_BUFF_SIZE]={0};        /* 读缓冲区 */
BYTE WriteBuffer[] =              /* 写缓冲区*/
"welcome using STM32\r\n"; 
extern  SD_CardInfo SDCardInfo;

static void FileDataToHex(unsigned char* data, uint16_t size, unsigned char* output);
...
/*----------------------- 格式化测试 ---------------------------*/  
	...
	{
		printf("SD card without file system, enter the init process...\r\n");
	...
	{
		printf("SD card fomount fail:(%d)\r\n",res_sd);
		printf("maybe SD card ini fail.\r\n");
		while(1);
	}
	else
	{
...
extern uint16_t r_index;
const unsigned char music_name[15] = {"0:track00"};
void FileSystem_Test(void)
{
	uint8_t i;
	uint16_t size = SD_READ_BUFF_SIZE;
	uint8_t music_num = 0;
	unsigned char music_num_char[2]= {0};
	unsigned char music_file_name[15] = {"0:track00"};
	unsigned char file_name_end[] = {".mp3"};
	uint8_t open_fail = 0;

	while (1)
	{

		if (KEY_PE3_PRESS)	// Previous Song
		{
			while (KEY_PE3_PRESS == 1)
				;
			...
			strcpy(music_file_name, music_name);
			music_num_char[0] = (unsigned char)(music_num + '0');
			strcat(music_file_name, (const char *)music_num_char);
			strcat(music_file_name, (const char *)file_name_end);
			printf("play %s\r\n", &music_file_name[2]);

		}
		// if (KEY_PE4_PRESS | open_fail)	//Next Song
		else		//Next Song
		{
			open_fail = 0;
			while (KEY_PE4_PRESS == 1)
				;
		...
		// res_sd = f_open(&fnew, "0:track004.mp3", FA_OPEN_EXISTING | FA_READ); // track00x.mp3
		res_sd = f_open(&fnew, music_file_name, FA_OPEN_EXISTING | FA_READ); // track00x.mp3
		if(res_sd == FR_OK)
		{
			printf("playing...\r\n");
				...
				// if(res_sd==FR_OK)
				// {
				// 	printf("read byte total:%d\r\n",fnum);
				// 	printf("read data:\r\n%s \r\n", ReadBuffer);
				// }

				while(f_read(&fnew, ReadBuffer, sizeof(ReadBuffer), &fnum) == FR_OK )
				{
					for (r_index = 0; r_index < MP3_SD_BUFF; )
					{
						vs1053_send_music_data(&ReadBuffer[r_index], 32);	//uint: 2byte, 16*2
						r_index += 32;
					}
					if((KEY_PE3_PRESS | KEY_PE4_PRESS) || (fnum < SD_READ_BUFF_SIZE))
					{
						break;
					}
				}
